...
test("makeRelease publishes artifact + manifest + chain into the store", async () => {
  await withSandbox(async (dir) => {
    ...
    // published artifact is executable (exec bits are POSIX-only; on Windows
    // executability is by extension/content, and the runnable-artifact claim
    // is carried by the "ok artifact really runs" test below)
